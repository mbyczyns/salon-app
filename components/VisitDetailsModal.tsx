  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
        ...
        {/* NAGŁÓWEK */}
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-200/50">
          <div>
            <h3 className="text-2xl font-[family-name:var(--font-oswald-bold)] text-slate-800 tracking-wide">Szczegóły wizyty</h3>
            <p className="text-slate-600 font-[family-name:var(--font-oswald-light)] font-bold">{visit.date} • {visit.time} — {visit.endTime}</p>
          </div>
          <button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-900 rounded-full transition-colors flex items-center justify-center bg-white shadow-sm">
        ...
        <div className="p-8 flex flex-col gap-8 overflow-y-auto max-h-[85vh]">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="flex-1 flex gap-4 items-center">
              <div className="w-16 h-16 rounded-2xl bg-slate-200 flex items-center justify-center text-2xl font-[family-name:var(--font-oswald-bold)] text-slate-600">
                {visit.clientName?.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase()}
              </div>
              <div>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <h5 className="text-lg font-[family-name:var(--font-oswald-bold)] text-slate-700 uppercase tracking-wider underline decoration-slate-100 underline-offset-8">Informacje po wizycie</h5>
              <span className={`px-4 py-1 rounded-full text-[10px] uppercase font-[family-name:var(--font-oswald-bold)] tracking-widest ${visit.status === "Odbyta" ? "bg-emerald-100 text-emerald-600" : "bg-blue-100 text-blue-600"}`}>
                {visit.status}
              </span>
            </div>
            ...
            {visit.status === "Nadchodząca" && !isEditing ? (
              <div className="bg-blue-50/50 p-8 rounded-3xl border-2 border-dashed border-blue-100 flex flex-col items-center justify-center gap-4 text-center">
                ...
                <button
                  ...
                  className="px-8 py-3 bg-slate-400 text-white font-[family-name:var(--font-oswald-bold)] rounded-full uppercase tracking-widest hover:bg-slate-600 transition-all shadow-md hover:shadow-lg"
                >
                  Zakończ wizytę
                </button>
              </div>
              ...
              <div className="flex flex-col gap-6 animate-in slide-in-from-bottom-4 duration-300">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border-b-2 border-slate-100 py-2 focus:outline-none focus:border-slate-400 transition-colors text-xl font-[family-name:var(--font-oswald-light)]"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full border-2 border-slate-50 p-4 rounded-2xl focus:outline-none focus:border-slate-400 focus:bg-slate-50/20 transition-all text-lg font-[family-name:var(--font-oswald-light)] resize-none"
                  />
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      ...
                      className="text-xs text-slate-600 font-bold uppercase hover:underline flex items-center gap-1"
                    >
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                    ...
                    <button
                      onClick={addMockPhoto}
                      className="aspect-square rounded-xl border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-slate-300 hover:border-slate-600 hover:text-slate-600 hover:bg-slate-50/30 transition-all gap-1"
                    >
